...
def main():
    if len(sys.argv) != 3:
        ...
        print("- dominant time 'tau' [ps]. Other taus which would give the same value as exp(-iEtau/hbar) will be used in this script.")
        sys.exit()

    T = float(sys.argv[1])
    dominant_tau = float(sys.argv[2])
    dominant_exponent = exp(-1j*E*dominant_tau/h_bar)
    ...
    for filename in glob.glob("data_*"):
        if '_T=' + str(T) + '_' in filename:
            tau = float(filename.split('_')[2][4:])
            candidate_exponent = exp(-1j*E*tau/h_bar)
            if np.isclose(dominant_exponent, candidate_exponent):
                print("Weszło!")
                # ... TO DO
                sys.exit()
