...
def main():
    ...
    for filename in glob.glob(input_file_prefix):
        if input_file_postfix in filename:
            ...
            if extended_mode:
                fig, axs = plt.subplots(nrows=6, ncols=3, figsize=(25,14))

                # First column
                axs[0][0].plot(taus, g_avs, "-", label=r'$g_{av}$')
                axs[0][0].plot(taus, g_avs_no_phase, "--", label=r'$g_{av}\ (e^{iE\tau/\hbar} = +1)$')
                axs[0][0].plot(taus, g_avs_negative_one, "--", label=r'$g_{av}\ (e^{iE\tau/\hbar} = -1)$')
                axs[0][0].plot(taus, g_avs_i, "--", label=r'$g_{av}\ (e^{iE\tau/\hbar} = +i)$')
                axs[0][0].plot(taus, g_avs_negative_i, "--", label=r'$g_{av}\ (e^{iE\tau/\hbar} = -i)$')

                axs[1][0].plot(taus, ps_plus, "-", label=r'$p_+$')
                axs[1][0].plot(taus, ps_minus, "-", label=r'$p_-$')
                axs[1][0].plot(taus, ps_plus_no_phase, "--", label=r'$p_+\ (no\ phase)$')
                axs[1][0].plot(taus, ps_minus_no_phase, "--", label=r'$p_-\ (no\ phase)$')

                axs[2][0].plot(taus, Ds_plus, "-", label=r'$D_+$')
                axs[2][0].plot(taus, Ds_minus, "-", label=r'$D_-$')
                axs[2][0].plot(taus, Ds_plus_no_phase, "--", label=r'$D_+\ (no\ phase)$')
                axs[2][0].plot(taus, Ds_minus_no_phase, "--", label=r'$D_-\ (no\ phase)$')
                ...
                # Second column
                axs[0][1].plot(taus, W_0s_real, "-", label=r'$Re(W_0)$')
                axs[0][1].plot(taus, W_0s_imag, "-", label=r'$Im(W_0)$')

                axs[1][1].plot(taus, W_1s_real, "-", label=r'$Re(W_1)$')
                axs[1][1].plot(taus, W_1s_imag, "-", label=r'$Im(W_1)$')
                axs[1][1].plot(taus, W_1s_phased_real, "-", label=r'$Re(e^{(...)}W_1)$')
                axs[1][1].plot(taus, W_1s_phased_imag, "-", label=r'$Im(e^{(...)}W_1)$')

                axs[2][1].plot(taus, W_2s_real, "-", label=r'$Re(W_2)$')
                axs[2][1].plot(taus, W_2s_imag, "-", label=r'$Im(W_2)$')
                axs[2][1].plot(taus, W_2s_phased_real, "-", label=r'$Re(e^{(...)}W_2)$')
                axs[2][1].plot(taus, W_2s_phased_imag, "-", label=r'$Im(e^{(...)}W_2)$')

                axs[3][1].plot(taus, W_3s_real, "-", label=r'$Re(W_3)$')
                axs[3][1].plot(taus, W_3s_imag, "-", label=r'$Im(W_3)$')

                axs[4][1].plot(taus, pure_phases_real, "-", label=r'$Re(exp(iE\tau)/\hbar))$')
                axs[4][1].plot(taus, pure_phases_imag, "-", label=r'$Im(exp(iE\tau)/\hbar))$')
                ...
                axs[4][1].grid()
                axs[4][1].legend()
                axs[4][1].set_xlabel(r'$\tau$')

                # Third column
                # print("Re(W_0 + W_3): ", W_0s_real + W_3s_real)
                axs[0][2].plot(taus, [x.real for x in W_0s + W_3s], "-", label=r'$Re(...),\ simple$')
                axs[0][2].plot(taus, [x.imag for x in W_0s + W_3s], "-", label=r'$Im(...),\ simple$')
                axs[0][2].plot(taus, W_0s_real + W_3s_real, "-", label=r'$Re(...),\ splitted$')
                axs[0][2].plot(taus, W_0s_imag + W_3s_imag, "-", label=r'$Im(...),\ splitted$')
                axs[0][2].set_title(r"$W_0 + W_3$")

                axs[1][2].plot(taus, [x.real for x in W_1s_phased + W_2s_phased], "-", label=r'$Re(...),\ simple$')
                axs[1][2].plot(taus, [x.imag for x in W_1s_phased + W_2s_phased], "-", label=r'$Im(...),\ simple$')
                axs[1][2].plot(taus, W_1s_phased_real + W_2s_phased_real, "-", label=r'$Re(...)$')
                axs[1][2].plot(taus, W_1s_phased_imag + W_2s_phased_imag, "-", label=r'$Im(...)$')
                axs[1][2].set_title(r"$e^{-i...}W_1 + e^{i...}W_2$")

                axs[2][2].plot(taus, W_0s_real + W_1s_phased_real + W_2s_phased_real + W_3s_real, "-", label=r'$Re(...)$')
                axs[2][2].plot(taus, W_0s_imag + W_1s_phased_imag + W_2s_phased_imag + W_3s_imag, "-", label=r'$Im(...)$')
                axs[2][2].set_title(r"$W_0 + e^{-i...}W_1 + e^{i...}W_2 + W_3$")

                Ws_sum = W_0s + W_1s_phased + W_2s_phased + W_3s
                Ws_sum_real = W_0s_real + W_1s_phased_real + W_2s_phased_real + W_3s_real
                Ws_sum_imag = W_0s_imag + W_1s_phased_imag + W_2s_phased_imag + W_3s_imag

                axs[3][2].plot(taus, np.abs(Ws_sum), "-", label="simple")
                axs[3][2].plot(taus, np.sqrt(Ws_sum_real**2 + Ws_sum_imag**2), "--", label="splitted")
                axs[3][2].set_title(r"$|W_0 + e^{-i...}W_1 + e^{i...}W_2 + W_3|$")

                axs[4][2].plot(taus, np.abs(Ws_sum) / np.abs(ps_plus), "-", label="simple")
                axs[4][2].plot(taus, np.sqrt(Ws_sum_real**2 + Ws_sum_imag**2) / np.abs(ps_plus), "--", label="splitted")
                axs[4][2].set_title(r"$|W_0 + e^{-i...}W_1 + e^{i...}W_2 + W_3| / |p_+|$")

                axs[5][2].plot(taus, ps_plus, "-", label=r'$p_+$')
                axs[5][2].plot(taus, pure_phases_real, "--", label=r'$Re(exp(iE\tau)/\hbar))$')
                axs[5][2].plot(taus, pure_phases_imag, "--", label=r'$Im(exp(iE\tau)/\hbar))$')
